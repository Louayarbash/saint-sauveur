    ...
    const alert = await this.alertController.create({
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: () => {
            this.featureService.deleteItem(this.item.images,this.item.id, 'rent-sale')
            .then(
              () => {
  ...
  deletePhoto(index : number){
        const loading = this.featureService.presentLoadingWithOptions(5000);
         if(this.postImages[index].storagePath !== '') {      
          ...
          this.featureService.updateItemWithoutOptions(this.item, 'rent-sale').then(()=> {
...
selectImageSource() {
  return this.featureService.selectImageSource(10, this.postImages.length, this.postImages, this.updateItemForm)
}
