    ...
    const alert = await this.alertController.create({
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: () => {
            this.featureService.deleteItem(this.item.files, this.item.id, 'regulations')
            .then(
              () => {
                this.featureService.presentToast(this.featureService.translations.DeletedSuccessfully,2000);
                this.dismissModal();
                this.router.navigate(['app/start-menu/regulation']);
              },
              err => { 
                console.log(err);
...
/*      if(this.item.category == 'announcement'){
  ...
  deleteFile(index : number){ 
        //const loading = this.featureService.presentLoadingWithOptions(2000).then( res => {return res;} );
         ...
         this.featureService.updateItemWithoutOptions(this.item, 'regulations').then(()=> {
