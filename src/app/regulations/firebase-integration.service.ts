  ...
  public getCombinedItemStore(dataSource: Observable<FirebaseItemModel>): DataStore<FirebaseItemModel> {
    ...
    public createItem(itemData : any,files : Files[])/* : Promise<DocumentReference>*/ : any{    
        ...
        for (var i = 0; i < files.length; i++) {
          try {
            let uploaded = await this.featureService.uploadToStorage(files[i].fileData, res.id, 'application/pdf', '.pdf', 'pdf/regulations/'+ this.loginService.getBuildingId() + '/');
/*             console.log("createItem: state", uploaded.state);
  ...
  public async updateItem(itemData: FirebaseItemModel, files : Files[]): Promise<void> {
      ...
      for (var i = 0; i < files.length; i++) {
          ...
          if (files[i].filePath != ""){
          try {
          let uploaded = await this.featureService.uploadToStorage(files[i].fileData, itemData.id, 'application/pdf', '.pdf', 'pdf/regulations/' + this.loginService.getBuildingId() + '/');
/*           console.log("updateItem: state", uploaded.state);
  ...
  private getItem(itemId: string): Observable<FirebaseItemModel> {
    return this.afs.doc<FirebaseItemModel>('regulations/' + itemId)
    .snapshotChanges()
    .pipe(
      map(a => {
