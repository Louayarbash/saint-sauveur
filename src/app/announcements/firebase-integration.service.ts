  ...
  */
  ...
  public getCombinedItemStore(dataSource: Observable<FirebaseItemModel>): DataStore<FirebaseItemModel> {
    ...
    public createItem(itemData : any,files : Files[])/* : Promise<DocumentReference>*/ : any{    
        ...
        for (var i = 0; i < files.length; i++) {
          try {
            let uploaded = await this.featureService.uploadToStorage(files[i].fileData, res.id, 'application/pdf', '.pdf', 'pdf/announcements/'+ this.loginService.getBuildingId() + '/');
  ...
  public async updateItem(itemData: FirebaseItemModel, files : Files[]): Promise<void> {
      ...
      for (var i = 0; i < files.length; i++) {
          ...
          if (files[i].filePath != ""){
          try {
          let uploaded = await this.featureService.uploadToStorage(files[i].fileData, itemData.id, 'application/pdf', '.pdf', 'pdf/announcements/' + this.loginService.getBuildingId() + '/');
  ...
  private getItem(itemId: string): Observable<FirebaseItemModel> {
    return this.afs.doc<FirebaseItemModel>('announcements/' + itemId)
    .snapshotChanges()
    .pipe(
      map(a => {
